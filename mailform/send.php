...
$conf = [];
$posted_form_type = $_POST['form_type'] ?? '';
$form_type = is_scalar($posted_form_type) ? trim((string)$posted_form_type) : '';

$conf['honeypot_field'] = 'website_url';
$conf['started_at_field'] = 'form_started_at';
$conf['csrf_field'] = 'csrf_token';
...
$form_routes = yafuso_mailform_routes($form_type);
$conf['thanks'] = $form_routes['thanks'];
$conf['failure'] = $form_routes['failure'];
...
$conf['field_labels'] = [
    ...
    'email' => 'メールアドレス',
    'tel' => '電話番号',
    'reservation_date' => '予約希望日',
    'reservation_time' => '予約希望時間',
    'guests' => 'ご利用人数',
    'plan' => 'ご利用内容',
    'room_request' => 'ご希望のお部屋',
    'room' => 'ご希望の区画',
    'genre' => '開業予定のジャンル・メニュー概要',
    'message' => 'ご質問・備考',
    'agree' => '確認事項への同意',
];
...
foreach (explode(',', (string)$conf['mailto']) as $recipient) {
...
try {
    ...
    error_log('mailform error: ' . $e->getMessage());
    mailform_redirect($conf['failure']);
}
...
function send_mail() {
	...
	if(0 < is_array_count($_POST)) {
        ...
        $honeypot_value = $_POST[$honeypot_field] ?? '';
		$honeypot_filled = !is_scalar($honeypot_value) || "" !== trim((string)$honeypot_value);
        $started_at_value = $_POST[$started_at_field] ?? 0;
		$started_at = is_scalar($started_at_value) ? (int)$started_at_value : 0;
        $session_started_at = (int)($_SESSION['yafuso_mailform_started_at'][$conf['form_type']] ?? 0);
		$elapsed_seconds = ($session_started_at > 0) ? (time() - $session_started_at) : -1;
        ...
        if (!mailform_validate_submission($_POST, $conf)) {
            mailform_redirect($conf['failure']);
        }
		...
		foreach($_POST as $key =>  $val) {
			...
			if('0' == $conf['no_disp'] || "" != $val) {
				...
				if(
                    ...
                    $key_title = $conf['field_labels'][$key_title] ?? $key_title;
        ...
        if (true === $conf['admin_posted']) {
            ...
            $admin_posted_body .= "[ HOST NAME ] ".($remote_addr !== '' ? gethostbyaddr($remote_addr) : '')."\n";
            $admin_posted_body .= "[ IP ADDRESS ] ".$remote_addr."\n";
            $admin_posted_body .= "[ USER AGENT ] ".(string)($_SERVER["HTTP_USER_AGENT"] ?? '')."\n";
        ...
        if("" != $user_email) {
            ...
            $user_mailer->Send();
                ...
                // 管理者への受付が成功した後の自動返信失敗は、二重送信を誘発しないよう記録のみ行う。
                error_log('mailform auto-reply error: ' . $replyException->getMessage());
            }
        }

        $formType = (string)$conf['form_type'];
        unset(
            $_SESSION['yafuso_mailform_tokens'][$formType],
            $_SESSION['yafuso_mailform_started_at'][$formType]
        );
        $_SESSION['yafuso_mailform_success'][$formType] = true;
        mailform_redirect($conf['thanks']);
	}
}

function mailform_redirect($location) {
    $location = (string)$location;
    if ($location === '' || $location[0] !== '/' || substr($location, 0, 2) === '//') {
        $location = '/';
    }
...
function mailform_validate_submission(array $post, array $conf): bool {
    $formType = (string)($post['form_type'] ?? '');
    if ($formType !== (string)$conf['form_type']) {
        return false;
    }
    ...
    $csrfValue = $post[$conf['csrf_field']] ?? '';
    $sessionCsrf = $_SESSION['yafuso_mailform_tokens'][$formType] ?? '';
    if (
        !is_scalar($csrfValue)
        || $sessionCsrf === ''
        || !hash_equals((string)$sessionCsrf, (string)$csrfValue)
    ) {
        return false;
    }
    ...
    if ((string)($post['agree'] ?? '') !== '1') {
        return false;
    }

    // input/select項目は改行・制御文字を禁止し、メール本文の見出し偽装を防ぐ。
    $singleLineLimits = [
        'name' => 120,
        'email' => 254,
        'tel' => 40,
    ...
    foreach ($singleLineLimits as $field => $maxBytes) {
    ...
    // textareaだけは改行を許可するが、サイズとその他の制御文字は制限する。
    foreach (['genre' => 4000, 'message' => 5000] as $field => $maxBytes) {
        if (!array_key_exists($field, $post)) {
            continue;
        }
        $value = $post[$field];
        if (
            !is_scalar($value)
            || strlen((string)$value) > $maxBytes
            || preg_match('/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/', (string)$value)
        ) {
            return false;
        }
    }

    if (($post['form_type'] ?? '') === 'karaoke_reservation') {
        $date = DateTimeImmutable::createFromFormat('!Y-m-d', (string)$post['reservation_date'], new DateTimeZone('Asia/Tokyo'));
        $dateErrors = DateTimeImmutable::getLastErrors();
        if (
            !$date
            || ($dateErrors !== false && ($dateErrors['warning_count'] > 0 || $dateErrors['error_count'] > 0))
            || $date < new DateTimeImmutable('today', new DateTimeZone('Asia/Tokyo'))
        ) {
            return false;
        }

        $allowedTimes = ['19:00', '19:30', '20:00', '20:30', '21:00', '21:30', '22:00', '22:30', '23:00', '23:30', '00:00', '00:30', '01:00', '01:30'];
        $allowedPlans = ['カラオケ利用', '飲み放題付きカラオケ', '宴会・二次会', 'キッズルーム希望', '未定・相談したい'];
        $allowedRooms = ['指定なし', '4〜8名個室', '4〜10名個室', '8〜18名個室', '座敷個室', 'キッズスペース希望'];
        $guests = filter_var($post['guests'], FILTER_VALIDATE_INT, ['options' => ['min_range' => 1, 'max_range' => 18]]);
        if (
            $guests === false
            || !in_array((string)$post['reservation_time'], $allowedTimes, true)
            || !in_array((string)$post['plan'], $allowedPlans, true)
            || !in_array((string)($post['room_request'] ?? '指定なし'), $allowedRooms, true)
        ) {
            return false;
        }
    }

    if (($post['form_type'] ?? '') === 'vendor_inquiry') {
        $allowedRooms = ['8号室', '1号室', '4号室', '未定'];
        if (!in_array((string)$post['room'], $allowedRooms, true)) {
            return false;
        }
    }
...
function check_file() {
	...
	foreach($_FILES['file']['name'] as $key => $val) {
		...
		if("" != $file_name) {
			...
			//拡張子取得
			$exp = mb_strtolower((string)pathinfo($file_name, PATHINFO_EXTENSION));
